...
import { User, Building2, Briefcase, Save, ExternalLink } from 'lucide-react';
import { loadData, saveData, KEYS } from '../services/storage';
import type { UserProfile } from '../types';
import './Profile.css';

export default function Profile() {
  const [profile, setProfile] = useState<UserProfile>({ name: '', role: '', company: '' });
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    setProfile(loadData(KEYS.PROFILE, { name: '', role: '', company: '' }));
  }, []);
